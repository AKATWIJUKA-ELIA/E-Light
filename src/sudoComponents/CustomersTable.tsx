...
const CustomersTable: React.FC<DataTable> = ({ users, status }) => {
        ...
        const matchesStatus = statusFilter === true || user.isVerified === statusFilter;
        ...
        const HandleCheckboxChange=(UserId:Id<"customers">)=>{
        ...
        const HandleBlockMany = async (UserIds: Id<"customers">[]) => {
        if (UserIds.length === 0) {
                setNotification({
                        status: "error",
        ...
        const allIds = users.map(user => user._id).filter((id): id is Id<"customers"> => id !== undefined);
        ...
        const HandleDelete=(UserId:Id<"customers">)=>{
                console.log(UserId)
                // setproductId(UserId)
                // setisdelete(true)
        }
        ...
        return (
                ...
                {users.map((user) => (
                        ...
                        <TableCell className="text-center">{user.isVerified?"Active":"Inactive"}</TableCell>
                        ...
                        >
                          {user.isVerified?"Block":"Activate"}
                        </Button>
                        <Button
